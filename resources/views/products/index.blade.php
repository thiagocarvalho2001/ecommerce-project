                ...
                <div class="p-4">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <form action="{{ route('cart.add', $product->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="add-to bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded">
                                Add to Cart
                            </button>
                        </form>
                        <form action="{{ route('wishlist.add', $product->id) }}" method="post">
                            @csrf
                            <button type="submit" class="add-to bg-purple-500 hover:bg-purple-700 text-white font-semibold py-2 px-4 border border-transparent rounded transition-colors focus:outline-none">
                                Add to Wishlist
                            </button>
                        </form>
                    </div>
